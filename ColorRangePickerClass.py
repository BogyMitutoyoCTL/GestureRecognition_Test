    ...
    def __init__(self, cap, colorChangedEvent):
        ...
        def apply():
            if self.updateUI is not None:
                ...
                root.destroy()
                colorChangedEvent(self.lowerHSV, self.upperHSV)

        Label(root, text="Lower HSV Color", font=("Helvetica", 16)).grid(row=0, column=0, padx=5)
        Label(root, text="Upper HSV Color", font=("Helvetica", 16)).grid(row=5, column=0, padx=5)

        self.w1 = Scale(root, from_=1, to=360, width=30, length=200, orient=HORIZONTAL)
        self.w1.set(95)
        self.w1.grid(row=1, column=0, padx=20, sticky=S)

        self.w2 = Scale(root, from_=1, to=100, width=30, length=200, orient=HORIZONTAL)
        self.w2.set(25)
        self.w2.grid(row=2, column=0, padx=20)

        self.w3 = Scale(root, from_=1, to=100, width=30, length=200, orient=HORIZONTAL)
        self.w3.set(30)
        self.w3.grid(row=3, column=0, padx=20, sticky=N)

        self.lowerColorCanvas = Canvas(root, width=200)
        self.lowerColorCanvas.grid(row=1, column=1, rowspan=3)
        self.upperColorCanvas = Canvas(root, width=200)
        self.upperColorCanvas.grid(row=6, column=1, rowspan=3)

        self.w4 = Scale(root, from_=1, to=360, width=30, length=200, orient=HORIZONTAL)
        self.w4.set(151)
        self.w4.grid(row=6, column=0, padx=20, sticky=S)

        self.w5 = Scale(root, from_=1, to=100, width=30, length=200, orient=HORIZONTAL)
        self.w5.set(100)
        self.w5.grid(row=7, column=0, padx=20)

        self.w6 = Scale(root, from_=1, to=100, width=30, length=200, orient=HORIZONTAL)
        self.w6.set(100)
        self.w6.grid(row=8, column=0, padx=20, sticky=N)

        self.b = Button(root, text="apply new range", command=apply, width=150, height=1)
